...
const DeliveryList = () => {
  ...
  const handleReturned = (bookingId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosPublic.patch(`/bookingReturned/${bookingId}`).then((res) => {
  ...
  const handleBooked = (bookingId, id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosPublic.patch(`/delivered/${bookingId}`).then((res) => {
          if (res.data.modifiedCount > 0) {
    ...
    axiosPublic.patch(`/totalDeliveredNumber/${id}`).then((res) => {
      if (res.data.modifiedCount > 0) {
        refetch()
      }
    })
  ...
  return (
    <div>
      <Helmet>
        <title>ParcelPioneer || DeliveryList</title>
      </Helmet>
                ...
                <td className="border border-slate-300">
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          <p className="text-center font-bold mb-4">See your delivery location</p>
                        </AlertDialogTitle>
                      </AlertDialogHeader>

                      {/* Fixed: Wrap the marker object in an array */}
                      <div className="h-[300px] w-full">
                        <DynamicMap
                          center={[Number.parseFloat(item?.latitude) || 0, Number.parseFloat(item?.longitude) || 0]}
                          zoom={13}
                          markers={[
                            {
                              position: [
                              ...
                              title: item?.place || "Delivery Location",
                            },
                          ]}
                        />
                      </div>
                ...
                <td className="border border-slate-300">
                  {item.status == "returned" || item.status == "delivered" ? (
                    ...
                    <span className="flex justify-evenly">
                      ...
                      <button>
                        <IoMdDoneAll
                          onClick={() => handleBooked(item._id, userRole._id)}
                          className="text-xl bg-[#3EA570] p-1 rounded-full text-white"
                        />
                      </button>
                    </span>
                  )}
